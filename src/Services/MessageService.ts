...
class MessageReturnServices {
  ...
  getMessages = async (userId: string, contactId: string, page: number = 0) => {
    return await axios.get<{ result: MessageResponse[] }>(
      `${this.route}/conversation/${userId}/${contactId}?page=${page}`,
    );
  };
  ...
  getUnreadCounts = async (userId: string) => {
    return await axios.get<Record<string, number>>(
      `${this.route}/unreadcounts/${userId}`,
    );
  };
  ...
  markAsRead = async (userId: string, contactId: string) => {
    return await axios.post(
      `${this.route}/markasread/${userId}/${contactId}`,
      {},
    );
  };
}
